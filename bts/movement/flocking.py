...
def cohesion(pos, nbs_pos, strength):
    ...
    """
    if len(nbs_pos) > 0:                            # v_c = 0 if no neighbours
        center = np.mean(np.array(nbs_pos), 0)
        v_c = strength*(center - pos)
    else: v_c = np.zeros(pos.shape)
    return v_c
...
def separation(pos, nbs_pos, strength):
    ...
    """
    if len(nbs_pos) > 0:                            # v_s = 0 if no neighbours
        v_s = strength*(len(nbs_pos)*pos-np.sum(np.array(nbs_pos), 0))
    else: v_s = np.zeros(pos.shape)
    return v_s
...
def alignment(vel, nbs_vel, strength):
    ...
    """
    if len(nbs_vel) > 0:                            # v_a = 0 if no neighbours
        avge_vel = np.mean(np.array(nbs_vel), 0)
        v_a = strength*(avge_vel-vel)
    else:
        v_a = np.zeros(vel.shape)
    return v_a
...
def update_vel(agent):
    ...
    new_vel = agent.p.speed*normalise(sum_vels(agent.vel, v_c, v_s, v_a))
    return new_vel